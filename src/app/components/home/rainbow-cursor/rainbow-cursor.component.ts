...
@Component({
    selector: 'fausto-rainbow-cursor',
    templateUrl: './rainbow-cursor.component.html',
    styleUrls: ['./rainbow-cursor.component.scss']
})
export class RainbowCursorComponent implements OnInit {
    ...
    private bubbleSize = 120;
    // the current mouse position
    public mousePos = { x: -1000, y: -1000 };
    ...
    public shouldHeight = (isPlatformBrowser(this.platform)) ? window.innerHeight : 0;
    @Input() set contentHeight(value: string) {
        this.shouldHeight = Number.parseInt(value);
    }
     ...
     */
    @HostListener('window:mousemove', ['$event']) mouseMove(event: any) {
        if (event instanceof MouseEvent) {
            this.drawFadingCircle(event.clientX, this.scrollY + event.clientY); // draws the brush
        } else {
            this.drawFadingCircle(event.x, this.scrollY + event.y);
        }
        ...
        this.timer = window.setTimeout(() => this.mouseStop(), 500);
    }


    @Input() set panMove(value: { x: number; y: number; }) {
        if (value.x !== 0 && value.y !== 0) this.mouseMove(value);
    }
    @Input() set panStop(val: number) {
        this.mouseStop()
    }
    ...
    drawFadingCircle(x: number, y: number) {
        let colorVector = this.rainbowColors[Math.round(this.currentColorPos) % this.fadeLength];
        ...
        if (dist < 300 && this.prevScrollY === Math.round(this.scrollY)) {
        ...
        this.currentColorPos += 0.5; // this controls how fast the colors will change
    }
    ...
    generateGradient() {
        ...
        GRADIENT_CANVAS.width = this.fadeLength;
    ...
    animationLoop() {
        ...
        if (this.CANVAS.height !== this.shouldHeight) this.CANVAS.height = this.shouldHeight;

        // Update the width of the canvas if it doesn't match
